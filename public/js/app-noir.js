/* ════════════════════════════════════════════════════════════════
   fvck it. — front-end logic (noir design)
   ----------------------------------------------------------------
...
function renderShop() {
  ...
  if (!products.length) {
    grid.innerHTML = `<div class="empty-shop">nothing here yet — add your first piece in the back office.</div>`;
    return;
  }
  grid.innerHTML = products.map((p, idx) => {
    ...
    <article class="card">
      ...
      <div class="card-body">
        <span class="look-no">LOOK ${no}</span>
        <h3>${p.name}</h3>
...
function renderCart() {
  ...
  $("cartItems").innerHTML = cart.length ? cart.map((c, i) => `
    ...
    : `<p class="muted" style="padding:40px 0;text-align:center;font-family:ui-monospace,Menlo,monospace;font-size:12px">your cart is empty.</p>`;
}
